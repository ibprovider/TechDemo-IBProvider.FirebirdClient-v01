...
//! \file    remote_fb__p13_lazy_send__srv_resource_helper.cpp
...
void RemoteFB__P13_LAZY_SEND__SrvResourceHelper::ProcessDeferReleasedResources__STMT
...
//------------------------------------------------------------------------
void RemoteFB__P13_LAZY_SEND__SrvResourceHelper::ProcessDeferReleasedResources
                                           (RemoteFB__ConnectorData* const pData)
{
 assert(pData!=nullptr);
...
//helper methods ---------------------------------------------------------
void RemoteFB__P13_LAZY_SEND__SrvResourceHelper::Helper__DropStmts
                                           (RemoteFB__ConnectorData* const pData)
{
 assert(pData);
 ...
 const wchar_t* const c_bugcheck_src
  =L"RemoteFB__P13_LAZY_SEND__SrvResourceHelper::Helper__DropStmts";
 ...
 try // сторож сбоев работы с портом.
 {
  { //--------------------------------------- 1. отправка пакетов
   ...
   { //-------------------------------------- 1.1 DROP STMT
    ...
    packet1.operation=protocol::set02::op_free_statement;
        ...
        spStmt=spStmt->m_pNextCnStmt DEBUG_CODE( _LITER_COMMA_ ++iDeletedStmt ) )
    {
     ...
     pData->GetPort()->send_packet_with_delay(portOpCtx,
                                              const_cast<const protocol::set02::PACKET_V02&>(packet1));
    }//for pStmt
   ...
   { //-------------------------------------- 1.2 проталкивающий пакет
    ...
    packet2.operation = protocol::set02::op_info_database;
      ...
      if(const t_ibp_error_element::self_ptr
          spErrRec
           =pset02::RemoteFB__PSET02__ErrorUtilites::BuildServerErrorRecord
             (pData,
              protocol::set02::op_free_statement,
              packet1.p_resp,
              E_FAIL))
      {
...
//------------------------------------------------------------------------
void RemoteFB__P13_LAZY_SEND__SrvResourceHelper::Helper__ReleaseBlobs(RemoteFB__ConnectorData* pData)
{
 assert(pData);
 assert(pData->GetPort());
 ...
 //-----------------------------------------
 const wchar_t* const c_bugcheck_src
  =L"RemoteFB__P13_LAZY_SEND__SrvResourceHelper::Helper__ReleaseBlobs";
  ...
  { //--------------------------------------- 1. отправка пакетов
   ...
   { //-------------------------------------- 1.1 CLOSE/CANCEL BLOB
        ...
        spBlob=spBlob->m_pNextCnNClosedBlob DEBUG_CODE( _LITER_COMMA_ ++iDeletedBlob) )
    {
     ...
     {
      packet1.operation=protocol::set02::op_close_blob;
     ...
     {
      packet1.operation=protocol::set02::op_cancel_blob;
   ...
   { //-------------------------------------- 2.1 CLOSE/CANCEL BLOB
        ...
        DEBUG_CODE(++iDeletedBlob ) )
    {
     ...
     if(packet1.operation==protocol::set02::op_response)
     {
      ...
      protocol::set02::P_OP operation;
      ibp_msg_code_type     msg_code_0;

      if(spBlob->m_ID.is_wait_close())
      {
       operation  =protocol::set02::op_close_blob;
      ...
      if(spBlob->m_ID.is_wait_cancel())
      {
       operation  =protocol::set02::op_cancel_blob;
...
//------------------------------------------------------------------------
void RemoteFB__P13_LAZY_SEND__SrvResourceHelper::Helper__ThrowBugCheck__UnkBlobHandleState
