////////////////////////////////////////////////////////////////////////////////
//! \ingroup db_client__remote_fb__api_p12
//! \file    remote_fb__api_p12__trans__rollback.h
//! \brief   Откат транзакции.
//! \author  Kovalenko Dmitry
//! \date    17.02.2015
#ifndef _remote_fb__api_p12__trans__rollback_H_
#define _remote_fb__api_p12__trans__rollback_H_

#include "source/db_client/remote_fb/api/remote_fb__api__trans__rollback.h"

namespace lcpi{namespace ibp{namespace db_client{namespace remote_fb{namespace api{namespace p12{
////////////////////////////////////////////////////////////////////////////////
//! \addtogroup db_client__remote_fb__api_p12
//! @{
////////////////////////////////////////////////////////////////////////////////
//class RemoteFB__API_P12__Rollback

/// <summary>
///  Откат транзакции.
/// </summary>
class RemoteFB__API_P12__Rollback LCPI_CPP_CFG__CLASS__FINAL
 :public REMOTE_FB__DEF_INTERFACE_IMPL_STATIC(RemoteFB__API__Rollback)
{
 private:
  using self_type=RemoteFB__API_P12__Rollback;

  RemoteFB__API_P12__Rollback(const self_type&)=delete;
  self_type& operator = (const self_type&)=delete;

 private:
  RemoteFB__API_P12__Rollback();

  virtual ~RemoteFB__API_P12__Rollback();

 public:
  /// Единственный экземпляр класса
  static self_type Instance;

  //Interface ------------------------------------------------------------

  /// <summary>
  ///  Откат транзакции.
  /// </summary>
  //! \param[in] pData
  //!  Not null.
  //! \param[in] pTrHandle
  //!  Not null. На входе должен указывать на ненулевое значение.
  //!
  //! В случае успешного отката транзакции, в (*pTrHandle) будет сброшен ID.
  virtual void exec(RemoteFB__ConnectorData* pData,
                    tr_handle_type*          pTrHandle)LCPI_CPP_CFG__METHOD__OVERRIDE_FINAL;
};//class RemoteFB__API_P12__Rollback

////////////////////////////////////////////////////////////////////////////////
//! @}
////////////////////////////////////////////////////////////////////////////////
}/*nms p12*/}/*nms api*/}/*nms remote_fb*/}/*nms db_client*/}/*nms ibp*/}/*nms lcpi*/
#endif
