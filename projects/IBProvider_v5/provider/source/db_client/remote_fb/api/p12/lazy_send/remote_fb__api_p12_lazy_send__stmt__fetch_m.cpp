...
//! \file    remote_fb__api_p12_lazy_send__stmt__fetch_m.h
//! \brief   API for fetching the row of result set. The version for the lazy_send-protocol.
...
////////////////////////////////////////////////////////////////////////////////
//class RemoteFB__API_P12_LAZY_SEND__FetchStatement_M

RemoteFB__API_P12_LAZY_SEND__FetchStatement_M RemoteFB__API_P12_LAZY_SEND__FetchStatement_M::Instance;

//------------------------------------------------------------------------
RemoteFB__API_P12_LAZY_SEND__FetchStatement_M::RemoteFB__API_P12_LAZY_SEND__FetchStatement_M()
{
}
...
//interface --------------------------------------------------------------
bool RemoteFB__API_P12_LAZY_SEND__FetchStatement_M::exec
                                            ...
                                            const RemoteFB__OutMsg_v1* const pOutMsg)
{
 ...
 const wchar_t* const c_bugcheck_src
  =L"RemoteFB__API_P12_LAZY_SEND__FetchStatement_M::exec";
 ...
 //«апрос должен быть прив€зан к транзакции.
 //»наче считаетс€ что он находитс€ в закрытом состо€нии.
 if((*pStmtHandle)->m_pParentTr==nullptr)
 {
  //[2015-09-08]
  // Ќа текущий момент это очень жестоко. ¬озможно в будущем
  // IBProvider научитс€ отслеживать закрытие курсоров через завершение транзакций.
  //assert(false);
 ...
 if(!(*pStmtHandle)->m_spFetchResult)
 {
  ...
  {
   cBatchRows
    =RemoteFB__P12__StmtHelper::ComputeBatchSize(pOutMsg->data.size());
  }
  else
  {
   const size_t
   ...
   cBatchRows
    =RemoteFB__P12__StmtHelper::ComputeBatchSize(szMsgData);
  }//else
 ...
 else
 {
  ...
  //! \todo
  //!  ѕроверка стабильности количества и описаний в pOutXSQLDA.
  //!  ≈сли вы€влены изменени€, то выкидывать BUG-CHECK ошибку.
 ...
 //!  - допустимость типов (нельз€ запрашивать SQL_NULL)
 ...
 for(DEBUG_CODE(bool debug__OpFetchWasSended=false);;)
 {
  ...
  if((*pStmtHandle)->m_spFetchResult->m_State==RemoteFB__FetchResult::state__eof)
  {
   ...
   //Ётот объект нам больше не нужен
  ...
  RemoteFB__P12_LAZY_SEND__StmtHelper::FetchNextRows
   (serverOperation,
    pData,
    *pStmtHandle); //throw
  ...
  //запрос Ќ≈ ЅџЋ закрыт (закрытие сопровождаетс€ выкидыванием исключени€).
