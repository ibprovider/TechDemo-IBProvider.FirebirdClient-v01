                                           ...
                                           (const isc_api::XSQLDA_V1* const pXSQLDA)
{
 ...
 //-----------------------------------------
 //вычисляем размер буфера под BLR
  ...
  try
  {
   ...
   switch(pXVar->get_typeID())
   {
    ...
    default:
    {
     ...
     //[2015-05-13]
     // Мы не должны сюда попадать в принципе.
     // Неизвестные типы данных отлавливаются на уровне Helper__Build_XSQLDA_BLR__CalcBufSize
     assert(false);
