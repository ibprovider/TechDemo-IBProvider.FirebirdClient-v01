...
//! \file    remote_fb__connector__init_p11_lazy_send.cpp
//! \brief   Инициализация 11-го протокола [LAZY SEND MODE].
...
#include "source/db_client/remote_fb/api/p12/lazy_send/remote_fb__api_p12_lazy_send__stmt__allocate.h"
...
void RemoteFB__Connector::Helper__FinalInitialize__P11__lazy_send()
{
 assert(m_spData);
 assert(m_spData->GetPort());
 assert(m_spData->GetPort()->get_protocol_version()==protocol::FB_PROTOCOL_VERSION11);
 ...
 m_spData->m_API__DropStatement
  .init(&api::p12::RemoteFB__API_P12_LAZY_SEND__DropStatement::Instance);

 //-----------------------------------------
 m_spData->m_API__PrepareStatement
  .init(&api::p12::RemoteFB__API_P12_LAZY_SEND__PrepareStatement::Instance);
 ...
 m_spData->m_API__CloseStatement
  .init(&api::p12::RemoteFB__API_P12_LAZY_SEND__CloseStatement::Instance);
 ...
 m_spData->m_API__CloseBlob
  .init(&api::p12::RemoteFB__API_P12_LAZY_SEND__CloseBlob::Instance);

 //-----------------------------------------
 m_spData->m_API__CancelBlob
  .init(&api::p12::RemoteFB__API_P12_LAZY_SEND__CancelBlob::Instance);
 ...
 //-----------------------------------------------------------------------
 //cancel service - not supported
}//Helper__FinalInitialize__P11__lazy_send
