...
#ifndef _lcpi_lib_structure__t_const_string_CC_
#define _lcpi_lib_structure__t_const_string_CC_
                                            ...
                                            const_pointer const str)
{
 ...
 assert(sz<=structure::t_numeric_limits<size_t>::max_value()/sizeof(char_type));

 size_t total_sz=offsetof(self_type,m_items[1]);

 if(!append_memory_size(total_sz,sz*sizeof(char_type)))
  throw std::bad_alloc();
