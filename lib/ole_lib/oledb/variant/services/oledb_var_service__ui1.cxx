...
DEFINE_OLEDB_TYPE_SERVICE_OBJ(UI1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_ALIGN(UI1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_LENGTH(UI1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_BINARY_LENGTH(UI1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_SIZE(UI1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_CONVERTOR(UI1)

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_UI1::internal__get_data__ok
                                            ...
                                            void*       const pBuffer)const
{
  ...
  *reinterpret_cast<const value_type**>(pBuffer)=&dbvar.valUI1;           /*!*/
 }
 else
 {
  assert_s(sizeof(value_type)==sizeof(dbvar.valUI1));                     /*!*/
  assert(cbMemSize>=sizeof(value_type));

  *reinterpret_cast<value_type*>(pBuffer)=dbvar.valUI1;                   /*!*/
 }

 return S_OK;
}//UI1::internal__get_data__ok

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_UI1::get_element(DBVARIANT*   const dest,
                                              REFIID             /*ElementIID*/,
                                              ...
                                              const void*  const pElement)const
{
 ...
 assert_s(sizeof(value_type)==sizeof(dest->valUI1));

 assert(this->get_type()==DBTYPE_UI1);
 ...
 //--------
 memcpy(&dest->valUI1,pElement,sizeof(value_type));
...
//------------------------------------------------------------------------
HRESULT t_oledb_type_service_UI1::set_element                             //!!!
                                             ...
                                             status_type*  const pBinding_Status)const
{
 ...
 assert_s(sizeof(source.valUI1)==sizeof(value_type));                     //!!!
 ...
 DBVARIANT::value_ui1_n_type elementValue;                                //!!!

 const HRESULT cvt_hr=DBVariant__to_UI1_ex(pConvCtx,source,&elementValue);//!!!
...
//------------------------------------------------------------------------
std::wstring t_oledb_type_service_UI1::get_print_wstr(const DBVARIANT& dbvar)const
{
 assert(dbvar.wType==this->get_type());

 //-------
 std::wstring result(L"UI1[");
 ...
 //-------
 result+=L"[";
 result+=structure::t_str_args_traits<wchar_t>::arg(dbvar.valUI1);
 result+=L"]";
