...
DEFINE_OLEDB_TYPE_SERVICE_OBJ(I2)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_ALIGN(I2)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_LENGTH(I2)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_BINARY_LENGTH(I2)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_SIZE(I2)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_CONVERTOR(I2)

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I2::internal__get_data__ok
                                            ...
                                            void*       const pBuffer)const
{
  ...
  *reinterpret_cast<const value_type**>(pBuffer)=&dbvar.valI2;            /*!*/
 }
 else
 {
  assert_s(sizeof(value_type)==sizeof(dbvar.valI2));                      /*!*/
  assert(cbMemSize>=sizeof(value_type));

  *reinterpret_cast<value_type*>(pBuffer)=dbvar.valI2;                    /*!*/
 }

 return S_OK;
}//I2::internal__get_data__ok

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I2::get_element(DBVARIANT*   const dest,     //!!!
                                             REFIID             /*ElementIID*/,
                                             ...
                                             const void*  const pElement)const
{
 ...
 assert_s(sizeof(value_type)==sizeof(dest->valI2));                       //!!!

 assert(this->get_type()==DBTYPE_I2);                                     //!!!
 ...
 //--------
 memcpy(&dest->valI2,pElement,sizeof(value_type));                        //!!!
...
//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I2::set_element                              //!!!
                                             ...
                                             status_type*  const pBinding_Status)const
{
 ...
 assert_s(sizeof(source.valI2)==sizeof(value_type));                      //!!!
 ...
 DBVARIANT::value_i2_n_type elementValue;                                 //!!!

 const HRESULT cvt_hr=DBVariant__to_I2_ex(pConvCtx,source,&elementValue); //!!!
...
//------------------------------------------------------------------------
std::wstring t_oledb_type_service_I2::get_print_wstr(const DBVARIANT& dbvar)const
{
 assert(dbvar.wType==this->get_type());

 //-------
 std::wstring result(L"I2[");
 ...
 //-------
 result+=L"[";
 result+=structure::t_str_args_traits<wchar_t>::arg(dbvar.valI2);
 result+=L"]";
