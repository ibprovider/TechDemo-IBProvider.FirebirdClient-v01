...
DEFINE_OLEDB_TYPE_SERVICE_OBJ(I1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_ALIGN(I1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_LENGTH(I1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_BINARY_LENGTH(I1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_SIZE(I1)

//------------------------------------------------------------------------
DEFINE_OLEDB_TYPE_SERVICE_GET_CONVERTOR(I1)

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I1::internal__get_data__ok
                                            ...
                                            void*       const pBuffer)const
{
  ...
  *reinterpret_cast<const value_type**>(pBuffer)=&dbvar.valI1;            /*!*/
 }
 else
 {
  assert_s(sizeof(value_type)==sizeof(dbvar.valI1));                      /*!*/
  assert(cbMemSize>=sizeof(value_type));

  *reinterpret_cast<value_type*>(pBuffer)=dbvar.valI1;                    /*!*/
 }

 return S_OK;
}//I1::internal__get_data__ok

//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I1::get_element(DBVARIANT*   const dest,     //!!!
                                             REFIID             /*ElementIID*/,
                                             ...
                                             const void*  const pElement)const
{
 ...
 assert_s(sizeof(value_type)==sizeof(dest->valI1));                       //!!!

 assert(this->get_type()==DBTYPE_I1);                                     //!!!
 ...
 //--------
 memcpy(&dest->valI1,pElement,sizeof(value_type));                        //!!!
...
//------------------------------------------------------------------------
HRESULT t_oledb_type_service_I1::set_element                              //!!!
                                             ...
                                             status_type*  const pBinding_Status)const
{
 ...
 assert_s(sizeof(source.valI1)==sizeof(value_type));                      //!!!
 ...
 DBVARIANT::value_i1_n_type elementValue;                                 //!!!

 const HRESULT cvt_hr=DBVariant__to_I1_ex(pConvCtx,source,&elementValue); //!!!
...
//------------------------------------------------------------------------
std::wstring t_oledb_type_service_I1::get_print_wstr(const DBVARIANT& dbvar)const
{
 assert(dbvar.wType==this->get_type());

 //-------
 std::wstring result(L"I1[");
 ...
 //-------
 result+=L"[";
 result+=structure::t_str_args_traits<wchar_t>::arg(dbvar.valI1);
 result+=L"]";
